...
class GearmanClient(GearmanConnectionManager):
    ...
    def submit_multiple_jobs(
        self, jobs_to_submit, background=False, max_retries=0, **kwargs
    ):
        """
        ...
        requests_to_submit = [
            self._create_request_from_dictionary(
                job_info, background=background, max_retries=max_retries
            )
    ...
    def submit_multiple_requests(
        self, job_requests, wait_until_complete=True, poll_timeout=None
    ):
    ...
    def wait_until_jobs_accepted(self, job_requests, poll_timeout=None):
        ...
        def continue_while_jobs_pending(any_activity):
            ...
            return any(
                is_request_pending(current_request) for current_request in job_requests
            )

        self.poll_connections_until_stopped(
            self.connection_list, continue_while_jobs_pending, timeout=poll_timeout
        )
    ...
    def wait_until_jobs_completed(self, job_requests, poll_timeout=None):
        ...
        self.poll_connections_until_stopped(
            self.connection_list, continue_while_jobs_incomplete, timeout=poll_timeout
        )
    ...
    def get_job_statuses(self, job_requests, poll_timeout=None):
        ...
        for current_request in job_requests:
            current_request.status["last_time_received"] = current_request.status.get(
                "time_received"
            )
    ...
    def establish_request_connection(self, current_request):
        ...
        if not rotating_connections:
            ...
            self.request_to_rotating_connection_queue[current_request] = (
                rotating_connections
            )
        ...
        if not chosen_connection:
            raise ServerUnavailable(
                "Found no valid connections: %r" % self.connection_list
            )
