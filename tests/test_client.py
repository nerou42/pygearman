...
class ClientTest(_GearmanAbstractTest):
    ...
    def setUp(self):
        ...
        self.original_handle_connection_activity = (
            self.connection_manager.handle_connection_activity
        )
    ...
    def generate_job_request(self, submitted=True, accepted=True):
        ...
        if submitted and accepted:
            self.command_handler.recv_command(
                GEARMAN_COMMAND_JOB_CREATED, job_handle=current_request.job.handle
            )
            assert (
                current_request.job.handle in self.command_handler.handle_to_request_map
            )
    ...
    def test_establish_request_connection_complex(self):
        ...
        self.connection_manager.connection_list = [
            failed_connection,
            failed_then_retried_connection,
            good_connection,
        ]
        ...
        assert (
            current_request
            not in self.connection_manager.request_to_rotating_connection_queue
        )
    ...
    def test_auto_retry_behavior(self):
        ...
        current_request.max_connection_attempts = (
            self.connection_manager.expected_failures + 1
        )
    ...
    def test_multiple_fg_job_submission(self):
        ...
        def mark_jobs_created(rx_conns, wr_conns, ex_conns):
            for current_job in expected_job_list:
                self.command_handler.recv_command(
                    GEARMAN_COMMAND_JOB_CREATED, job_handle=current_job.handle
                )
        ...
        job_requests = self.connection_manager.submit_multiple_jobs(
            job_dictionaries, wait_until_complete=False
        )
    ...
    def test_single_bg_job_submission(self):
        ...
        def mark_job_created(rx_conns, wr_conns, ex_conns):
            self.command_handler.recv_command(
                GEARMAN_COMMAND_JOB_CREATED, job_handle=expected_job.handle
            )
            return rx_conns, wr_conns, ex_conns
    ...
    def test_wait_for_multiple_jobs_to_complete_or_timeout(self):
        ...
        def multiple_job_updates(rx_conns, wr_conns, ex_conns):
            ...
            if self.update_requests:
                self.command_handler.recv_command(
                    GEARMAN_COMMAND_WORK_COMPLETE,
                    job_handle=completed_request.job.handle,
                    data=b"12345",
                )
                self.command_handler.recv_command(
                    GEARMAN_COMMAND_WORK_FAIL, job_handle=failed_request.job.handle
                )
        ...
        finished_requests = self.connection_manager.wait_until_jobs_completed(
            [completed_request, failed_request, timeout_request], poll_timeout=0.01
        )
        del self.update_requests

        (
            finished_completed_request,
            finished_failed_request,
            finished_timeout_request,
        ) = finished_requests
        ...
        assert (
            finished_timeout_request.job.handle
            in self.command_handler.handle_to_request_map
        )
    ...
    def test_get_job_status_unknown(self):
        ...
        self.command_handler.recv_command(
            GEARMAN_COMMAND_WORK_FAIL, job_handle=current_handle
        )

        def retrieve_status(rx_conns, wr_conns, ex_conns):
            self.command_handler.recv_command(
                GEARMAN_COMMAND_STATUS_RES,
                job_handle=current_handle,
                known="0",
                running="0",
                numerator="0",
                denominator="1",
    ...
    def test_get_job_status_timeout(self):
        ...
        job_request = self.connection_manager.get_job_status(
            single_request, poll_timeout=0.01
        )
        assert job_request.timed_out
...
class ClientCommandHandlerInterfaceTest(_GearmanAbstractTest):
    ...
    def test_send_job_request(self):
        ...
        for priority in (PRIORITY_NONE, PRIORITY_HIGH, PRIORITY_LOW):
            for background in (False, True):
                ...
                queued_request = (
                    self.command_handler.requests_awaiting_handles.popleft()
                )
                assert queued_request == current_request

                expected_cmd_type = submit_cmd_for_background_priority(
                    background, priority
                )
    ...
    def test_get_status_of_job(self):
        ...
        self.assert_sent_command(
            GEARMAN_COMMAND_GET_STATUS, job_handle=current_request.job.handle
        )
...
class ClientCommandHandlerStateMachineTest(_GearmanAbstractTest):
    ...
    def generate_job_request(self, submitted=True, accepted=True):
        current_request = super(
            ClientCommandHandlerStateMachineTest, self
    ...
    def test_received_job_created(self):
        ...
        self.command_handler.recv_command(
            GEARMAN_COMMAND_JOB_CREATED, job_handle=new_handle
        )
    ...
    def test_received_job_created_out_of_order(self):
        ...
        with pytest.raises(InvalidClientState):
            self.command_handler.recv_command(
                GEARMAN_COMMAND_JOB_CREATED, job_handle=None
            )
    ...
    def test_required_state_queued(self):
        ...
        for bad_state in invalid_states:
            ...
            with pytest.raises(InvalidClientState):
                self.command_handler.recv_command(
                    GEARMAN_COMMAND_WORK_DATA, job_handle=job_handle, data=new_data
                )

            with pytest.raises(InvalidClientState):
                self.command_handler.recv_command(
                    GEARMAN_COMMAND_WORK_WARNING, job_handle=job_handle, data=new_data
                )
            ...
            with pytest.raises(InvalidClientState):
                self.command_handler.recv_command(
                    GEARMAN_COMMAND_WORK_COMPLETE, job_handle=job_handle, data=new_data
                )

            with pytest.raises(InvalidClientState):
                self.command_handler.recv_command(
                    GEARMAN_COMMAND_WORK_FAIL, job_handle=job_handle
                )
