...
@pytest.mark.parametrize(
    "keyfile,certfile,ca_certs,expected_use_ssl",
    [
...
def test_use_ssl_only_if_all_three_files(keyfile, certfile, ca_certs, expected_use_ssl):
    conn = connection.GearmanConnection(
        host="localhost", keyfile=keyfile, certfile=certfile, ca_certs=ca_certs
    )
...
def test_send_commands_to_buffer():
    ...
    conn._outgoing_commands.append(
        (GEARMAN_COMMAND_TEXT_COMMAND, {"raw_text": "raw---text"})
    )
    conn.send_commands_to_buffer()
