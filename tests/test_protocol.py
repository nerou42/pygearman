...
class TestProtocolBinaryCommands(object):
    ...
    def test_parsing_without_enough_data(self):
        ...
        not_enough_data_command_buffer = array.array(
            "b", not_enough_data_command_buffer
        )
        cmd_type, cmd_args, cmd_len = protocol.parse_binary_command(
            not_enough_data_command_buffer
        )
        ...
        not_enough_data_command_buffer = struct.pack(
            "!4sII", protocol.MAGIC_RES_STRING, protocol.GEARMAN_COMMAND_ECHO_RES, 4
        )
        not_enough_data_command_buffer = array.array(
            "b", not_enough_data_command_buffer
        )
    ...
    def test_parsing_no_args(self):
        noop_command_buffer = struct.pack(
            "!4sII", protocol.MAGIC_RES_STRING, protocol.GEARMAN_COMMAND_NOOP, 0
        )
    ...
    def test_parsing_single_arg(self):
        ...
        echo_command_buffer = struct.pack(
            "!4sII4s",
            protocol.MAGIC_RES_STRING,
            protocol.GEARMAN_COMMAND_ECHO_RES,
            4,
            echoed_string,
        )
    ...
    def test_parsing_single_arg_with_extra_data(self):
        ...
        excess_echo_command_buffer = struct.pack(
            "!4sII9s",
            protocol.MAGIC_RES_STRING,
            protocol.GEARMAN_COMMAND_ECHO_RES,
            4,
    ...
    def test_parsing_multiple_args(self):
        ...
        binary_payload = protocol.NULL_CHAR.join(
            [b"test", b"function", b"identifier", expected_data]
        )
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (protocol.GEARMAN_COMMAND_GRAB_JOB, {"extra": "arguments"}),
            ...
            (protocol.GEARMAN_COMMAND_JOB_CREATED, {"extra": "arguments"}),
            # Assert we get a non-string argument
            (protocol.GEARMAN_COMMAND_JOB_CREATED, {"job_handle": 12345}),
            # Assert we get a non-binary argument with any other non-binary type.
            (protocol.GEARMAN_COMMAND_JOB_CREATED, {"job_handle": FakeType()}),
            # Assert we check for NULLs in all but the "last" argument,
            # where last depends on the cmd_type.
            (
                protocol.GEARMAN_COMMAND_SUBMIT_JOB,
                {"task": b"funct\x00ion", "data": b"abcd", "unique": b"12345"},
            ),
            (
                protocol.GEARMAN_COMMAND_SUBMIT_JOB,
                {"task": b"function", "data": b"abcd", "unique": b"123\x00\x0045"},
            ),
        ],
    )
    ...
    def test_packing_errors_last_arg(self):
        ...
        protocol.pack_binary_command(
            ...
            cmd_args={"task": b"function", "data": b"ab\x00cd", "unique": b"12345"},
        )
    ...
    def test_packing_response(self):
        ...
        packed_command_buffer = protocol.pack_binary_command(
            cmd_type, cmd_args, is_response=True
        )
    ...
    @pytest.mark.parametrize(
        "cmd_args, cmd_resp",
        [
            ({"data": b"abcde"}, {"data": b"abcde"}),
            ({"data": "abcde"}, {"data": b"abcde"}),
            ({"data": "abcde"}, {"data": b"abcde"}),
        ],
    )
...
class TestProtocolTextCommands(object):
    ...
    def test_parsing_single_line(self):
        ...
        assert cmd_response == {
            "raw_text": compat.array_to_bytes(received_data).strip()
        }
    ...
    def test_parsing_multi_line(self):
        ...
        cmd_type, cmd_response, cmd_len = protocol.parse_text_command(received_data)
        assert cmd_type == protocol.GEARMAN_COMMAND_TEXT_COMMAND
        assert cmd_response == {"raw_text": compat.array_to_bytes(sentence_one).strip()}
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (protocol.GEARMAN_COMMAND_TEXT_COMMAND, {"bad_text": "abcdefghij"}),
        ],
    )
    def test_packing_errors(self, cmd_type, cmd_args):
        with pytest.raises(ProtocolError):
    ...
    def test_packing_single_line(self):
        ...
        cmd_args = {"raw_text": expected_string}
...
class GearmanCommandHandlerTest(_GearmanAbstractTest):
    ...
    def _test_send_command(self):
        ...
        self.command_handler.send_command(
            protocol.GEARMAN_COMMAND_ECHO_REQ, text="hello world"
        )
    ...
    def assert_sent_command(self, expected_cmd_type, **expected_cmd_args):
        ...
        handler_cmd_type, handler_cmd_args = (
            self.command_handler.sent_command_queue.popleft()
        )
        self.assert_commands_equal(handler_cmd_type, expected_cmd_type)
        assert handler_cmd_args == expected_cmd_args

        super(GearmanCommandHandlerTest, self).assert_sent_command(
            expected_cmd_type, **expected_cmd_args
        )
